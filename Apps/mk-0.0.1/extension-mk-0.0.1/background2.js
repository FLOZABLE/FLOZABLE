const serverEndPoint = 'https://flozable.com'
...
(async () => {
  let response = await fetch(`${serverEndPoint}/api/bring-tabs`, {
    method: 'POST',
    headers: {
...
//bring tab's setting
(async () => {
  let response = await fetch(`${serverEndPoint}/api/bring-activity-setting`, {
    method: 'POST',
    headers: {
      'Authorization': 'sdfwpep9p345oD563$SDFksdfkdswt9e9',
      'Content-Type': 'application/json'
...
async function update() {
  let response = await fetch(`${serverEndPoint}/api/update-tabs`, {
    method: 'POST',
    headers: {
      'Authorization': 'sdfwpep9p345oD563$SDFksdfkdswt9e9',
      'Content-Type': 'application/json'
...
async function updateTabSetting(domain, block, timer) {
  const tabSetting = tabSettings.find(tabSetting => { return tabSetting.domain === domain });
  if (!tabSetting) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.command === 'update-setting') {
    //const response = await updateTabSetting(message.domain, message.block, message.timer);
    (async() => {
      const response = await updateTabSetting(message.domain, message.block, message.timer);
