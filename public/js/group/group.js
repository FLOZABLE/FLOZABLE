...
countTags();
createTag();
console.log(document.querySelector('[id="8tXVpsb"]'))
const groupRemover = () => {
  groupList.forEach((group) => {
    if(tags.every((tag) => group.tags.includes(tag))){
      console.log(group.group_id)
      const selectedGroup = document.querySelector(`.groups-container .group[id="${group.group_id}"]`);
      selectedGroup.style.display = 'block';
    } else {
      console.log('no',group.group_id);
      const selectedGroup = document.querySelector(`.groups-container .group[id="${group.group_id}"]`);
