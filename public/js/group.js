...
const pwAskingModal = document.querySelector(".password-modal");
const mygroupsWrapper = document.querySelector(".mygroups-container");
const pwModalClosteBtn = document.querySelector(".password-modal #modalclosebtn");
const pwModalErr = document.querySelector(".password-modal .err-msg");

pwModalClosteBtn.addEventListener('click', () => {
  pwAskingModal.classList.add('closed');
})
...
const joinButtonEvent = async(joinLeaveButton, groupId, type) => {
  if(type == 'public') {
    ...
    pwAskingModal.classList.remove('closed');
    ...
    groupPwSubmitBtn.addEventListener('click', async() => {
      ...
      } else if (response.reason == 'no session') {
        window.location = "../account"
      } else if (response.reason == 'password wrong') {
        pwModalErr.classList.remove('closed');
      }
    })
  }
}
...
(async() => {
  ...
  userEmail = response[1];
  groupWithUser = response[2];
  console.log(groupList, userEmail, groupWithUser);
...
const groupsNotSearched = document.querySelector(".no-result");

groupsSearch.addEventListener("input", () => {
  const groups = document.querySelectorAll(".groups-container .group");
  const groupNames = document.querySelectorAll(".groups-container .group .name p");
  const groupExplanations = document.querySelectorAll(".groups-container .group .explanation");
  let searched = false;
  let groupValue = [];
  if(!groups.length) {
    return 0;
