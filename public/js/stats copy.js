...
(async () => {
  ...
  if(ranking.success) {
    ...
    rankingChart = new Chart(rankingGraphContainer, {
      type: "scatter",
      data: {
  ...
  // Doughnut chart
  subjectDoughnutChart = new Chart(ctx2, {
    type: "doughnut",
    data: {
  ...
  let histogramContainer = document.getElementById('timelineHistogramChart');

  timelineHistogramChart = new Chart(histogramContainer, {
    type: 'bar',
    data: {
  ...
  compareActivateBtn.addEventListener('click', () => {
    if(compareUserselected) {
      ...
      compareChart.data.labels = labels.map(date => {return `${new Date(date).getMonth() + 1}/${new Date(date).getDate()}`});
  ...
  if(activities.data[calendarDate / 1000]) {
    webUsageChart = new Chart(appUsageCont, {
      type: "doughnut",
      data: {
...
function updateCharts() {
  if(graphViewOpt == 'day') {
    ...
    //ranking graph
    rankingChart.data.labels = myRanking.daily.map((day, index) => {
      const date = new Date(todayTime - (myRanking.daily.length - index - 1) * 1000 * 60 * 60 * 24);
      return `${date.getMonth() + 1}/${date.getDate()}`
    })
    rankingChart.data.datasets[0].data = myRanking.daily;
    rankingChart.update();
    ...
    //ranking graph
    rankingChart.data.labels = myRanking.weekly.map((day, index) => {
      const date = new Date(todayTime - (myRanking.weekly.length - index - 1) * 1000 * 60 * 60 * 24 * 7);
      return `${date.getMonth() + 1}/${date.getDate()}`
    })
    rankingChart.data.datasets[0].data = myRanking.weekly;
    rankingChart.update();
    ...
    //ranking graph
    rankingChart.data.labels = myRanking.monthly.map((day, index) => {
      const date = new Date(todayTime - (Math.round(myRanking.monthly.length - index - 1) * 1000 * 60 * 60 * 24 * 30.4375));
      return `${date.getMonth() + 1}`
    })
    rankingChart.data.datasets[0].data = myRanking.monthly;
    rankingChart.update();
