...
function toggleTimer(timer) {
  if (timer.run) {
    ...
    timer.timer = setInterval(function () { count(timer); }, 1000);
    if(!timer.type) {
...
const createMemberTimer = (membersWrapper, member) => {
  ...
  setTimeout(() => {
    member.timer = {
      ...
      run: false,
      timer: null,
      secDisp: document.querySelectorAll(`#id${member.userId} .sec`),
      minDisp: document.querySelectorAll(`#id${member.userId} .min`),
      hrDisp: document.querySelectorAll(`#id${member.userId} .hr`),
      playBtn: null,
    }
...
socket.on('studying', (userId, groups) => {
  groups.forEach((group) => {
    ...
    const memberDivders = document.querySelectorAll(`.groups-container .swiper-slide#${group} #id${userId}`);
    console.log(memberDivders)
...
socket.on('addUser', async(group, userId) => {
  ...
  const roomWrapper = document.querySelectorAll(`.groups-container .swiper-slide#${group} ul `);
  const groupInfo = groupList.find(groupObj => groupObj.group_id == group);
