...
function SubjecTimer({ selectedSubject, setSelectedSubject }) {
    ...
    if (tutorial === 7) {
      setTimeout(() => {
        const { width, top, left } =
          chooseSubjectRef.current.getBoundingClientRect();
      ...
      setTimeout(() => {
        const { width, top, left, height } =
          subjectOptionsRef.current.getBoundingClientRect();
        tutorialBoxRef.current.style.left = left + "px";
        tutorialBoxRef.current.style.top = top + "px";
        tutorialBoxRef.current.style.width = width + "px";
      ...
      setTimeout(() => {
        const { width, top, left, height } =
          startSubjectRef.current.getBoundingClientRect();
        tutorialBoxRef.current.style.left = left + "px";
        tutorialBoxRef.current.style.top = top + "px";
        tutorialBoxRef.current.style.width = width + "px";
        tutorialBoxRef.current.style.height = height + "px";
  ...
  useEffect(() => {
    ...
    if (selectedSubject.default) {
      //is the default object, not previously selected subject
      setSelectedSubject(subjects[0]);
      setSubjectTimer({
        total: subjects[0].daily.total[subjects[0].daily.total.length - 1],
      });
    }
    const timerValueObj = {};
    subjects.map((subject) => {
      timerValueObj[subject.subject_id] =
          ...
          if (currentSubject.subject_id === subject.subject_id) {
            return {
              ...
              daily: {
                ...
                total: [
                  ...currentSubject.daily.total.slice(
                    0,
                    currentSubject.daily.total.length - 1
                  ),
                  timerValues[currentSubject.subject_id],
                ],
              },
            };
          }
  ...
  return (
    <div className={styles.SubjectTimer}>
      <div className={styles.timerWrapper} ref={chooseSubjectRef}>
        <button
          id="tutorial-7"
        ...
        <ul
          className={`${styles.options} customScroll`}
