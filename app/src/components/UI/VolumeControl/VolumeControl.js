...
function VolumeControl(props) {
  const [isMuted, setMuted] = useState(true); // Track whether the props.volume is muted
  const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 });
  const [prevVolume, setPrevVolume] = useState(props.volume);
  ...
  const soundPicto = isMuted ? (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" className={styles.soundPicto}>
  ...
  >
    ...
    />
    <p className={styles.volume}>{props.volume}</p>
  </div>
  );
}
