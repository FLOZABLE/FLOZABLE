...
function LineInput({ title, value, setValue, type }) {
  ...
  return (
    <div className={styles.LineInput}>
      <input type={type} onFocus={() => { setIsFocused(true) }} onBlur={() => { setIsFocused(false) }} defaultValue={value} onChange={(e) => {setValue(e.target.value)}}/>
      {title ? <div className={styles.title}>{title}</div> : ''}
      <div className={`${styles.lineContainer} ${isFocused ? styles.focused : ''}`}>
