    ...
    Object.keys(userInfo).forEach(async (info) => {
      if (info.includes("subject:")) {
        ...
        if (infoSubjectId === subjectId) {
          ...
          const push = await redisClient.rPush(
          ...
          const prevTimer = await redisClient.hGet(
            `user:${userId}`,
            "timerInfo"
          );
          if (prevTimer) {
            ...
            const missingTotal = Math.floor(
              (lastVal ? lastVal[1] : 0) / (MAXSTORELEN * 2)
            );
...
Router.post("/stop", async (req, res) => {
  autoSignin(req, res, async () => {
    ...
    const activeSubject = JSON.parse(
      await redisClient.hGet(`user:${userId}`, "ActiveSubject")
    );
    if (activeSubject.id === subjectId) {
      const activity = JSON.parse(
        await redisClient.rPop(`user:${userId}:subject:${subjectId}`)
      );
      const now = Math.floor(new Date().getTime() / 1000);
