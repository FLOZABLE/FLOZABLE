...
Router.post("/bring-plans", async (req, res) => {
  ...
  let [plans] = await connection.query(`SELECT id, title, start, end, \`repeat\`, description, notification, subject, priority, completed FROM plans where user_id = ?`, [req.session.user_id]);
...
Router.post('/update-plan', async (req, res) => {
  try {
    ...
    if (isValid) {
      const connection = pool.promise();
      try {
        const insertInfo = { ...planInfo, user_id: req.session.user_id };
        const [deletePrev] = await connection.query(`DELETE FROM plans WHERE user_id = ? AND id = ?`, [req.session.user_id, planInfo.id]);
